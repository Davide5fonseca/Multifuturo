...
<header class="border-b border-sand-200 bg-sand-50" x-data="{ open: false }" @keydown.escape.window="open = false">
    <div class="container-site flex h-20 items-center justify-between gap-8">
        ...
        <nav class="ml-auto hidden items-center gap-7 whitespace-nowrap xl:flex" aria-label="{{ __('ui.nav.main') }}">
        ...
        <a href="{{ route('favorites') }}" class="hidden items-center gap-2 text-sm text-ink hover:text-olive-700 xl:flex" x-data>
        ...
        <x-site.language-switcher class="ml-4 hidden xl:flex" />

        <button type="button" class="xl:hidden -mr-2 grid h-11 w-11 place-items-center text-ink" @click="open = !open" :aria-expanded="open" aria-controls="menu-movel">
